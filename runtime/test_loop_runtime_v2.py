...
class RuntimeV2(unittest.TestCase):
 def ready(self):
  l=RunLedger("LMS production verification",topology="HYBRID"); l.transition("ORIENTING"); l.transition("PLANNED"); l.transition("RUNNING"); l.transition("VERIFYING"); return l
 ...
 def test_real_redaction(self):
  e=EventLog("r"); x=e.emit("tool.called",payload={"patient":{"name":"SECRET"},"safe":"ok"},redacted_fields=["patient.name"])
  self.assertEqual(x["payload"]["patient"]["name"],"[REDACTED]"); self.assertNotIn("SECRET",str(x))
 def test_parallel_causality_explicit(self):
  e=EventLog("r"); a=e.emit("work.started",branch_id="A"); b=e.emit("work.started",branch_id="B")
  self.assertIsNone(b["parent_span_id"]); self.assertNotEqual(a["span_id"],b["span_id"])
 def test_checkpoint_semantic_guard(self):
  ...
  with tempfile.TemporaryDirectory() as d:
   cp=save_checkpoint(l,e,Path(d)); resume_checkpoint(Path(cp["uri"]),cp["checksum"],{"my_sha":"abc","lms_ssot":"v1"})
 ...
 def test_1000_lms_adversarial(self):
  ...
  for seed in range(1000):
   r=random.Random(seed); l=self.ready(); mission_rel=r.random()<.9
   f=l.add_failure(r.choice(classes),"LMS_VIDEO",r.choice(["S1","S2","S3"]),mission_rel)
   if mission_rel:
    self.assertFalse(l.can_complete()); f["state"]="CLOSED"; self.assertTrue(l.can_complete())
